...
public class AzureVaultExtensionTest {
    ...
    void verifyCreateVault(AzureVaultExtension extension, ServiceExtensionContext context) {
        ...
        Config cfg = mock();
        when(cfg.getString(eq(VAULT_NAME))).thenReturn(VAULT_NAME_SETTING);
        when(context.getConfig()).thenReturn(cfg);

        assertThat(extension.createAzureVault(context)).isInstanceOf(AzureVault.class);
        verify(context, atLeastOnce()).getConfig();
        verify(cfg).getString(eq(VAULT_NAME_SETTING));
    }
